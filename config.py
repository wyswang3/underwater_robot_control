...
def _pretty_dict(d: Dict[str, Any], indent: int = 2) -> str:
    return "\n".join(f"{' '*indent}{k:<20}: {v}" for k, v in d.items())
...
class TrainingConfig:
    # 模型选择：'hybrid', 'direct', 'physics'
    MODEL_TYPE: str = "hybrid"

    # 数据及模型相关参数
    WINDOW_SIZE: int = 9
    INPUT_DIM: int = 14  # PWM(8) + IMU(6)

    # Direct Mapping 隐藏维度
    DIRECT_P_HIDDEN: int = 256
    DIRECT_I_HIDDEN: int = 256
    FUSION_HIDDEN_DIM: int = 128

    # Thrust MLP 隐藏维度
    THRUST_MLP_HIDDEN_DIM: int = 32

    # Physics Net 编码器维度
    PHY_ENCODER_HIDDEN: int = 1024
    PHY_LAYERS: int = 1
    PHY_DROPOUT: float = 0.2

    # Residual MLP 隐藏维度
    RES_HIDDEN_DIM: int = 32
...
class Config:
    ...
    def print_config(self):
        print("\n========== Config ==========")
        print("• Paths")
        for k, v in asdict(self.paths).items(): print(f"  {k:<25}: {v}")
        print("• Training")
