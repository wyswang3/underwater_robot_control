...
def plot_segment_comparison(time_axis, measured, predicted, save_path=None):
    ...
    采用 Times New Roman 字体，字号增大1号，去掉网格，左列线条红/蓝，图例无边框，无子图标题。
    """
    # 全局字体设置
    # 在脚本开头，替换原来的 plt.rc(...)
    plt.rcParams.update({
    ...
    fig, axs = plt.subplots(
        nrows=num_axes, ncols=2,
        figsize=(14, 4 * num_axes),
        sharex="col"
    )
    ...
    for i in range(num_axes):
        # 左：Measured vs Predicted（红/蓝 細实/虚线）
        ax_ts = axs[i, 0]
        ax_ts.plot(time_axis, measured[:, i],  'o-',  color='red',  linewidth=1, label='Measured')
        ax_ts.plot(time_axis, predicted[:, i], 'o-', color='blue', linewidth=1, label='Predicted')
        ax_ts.set_ylabel(axis_names[i])
        if i < num_axes - 1:
        ...
        # 右：Residual（紫色 細实线 + 红色 虚线）
        ax_res = axs[i, 1]
        residual = predicted[:, i] - measured[:, i]
        ax_res.plot(time_axis, residual, '-', color='purple', linewidth=1, label='Residual')
